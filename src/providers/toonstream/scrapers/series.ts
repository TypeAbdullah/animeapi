...
import type { AnimeCard, Cast, Episode, Genre, Season, Tag } from "../lib/types.js";
...
export async function ScrapeSeriesInfo(url: string) {
    ...
    try {
        const html = await browserFetch(decodedURL);
        const $ = cheerio.load(html, { xml: true });
...
export async function seriesScraper(slug: string) {
    ...
    try {
        ...
        const toonStreamIframeUrls = [
            ...$("aside#aa-options iframe").map((_, el) => $(el).attr("data-src") || $(el).attr("src")).get(),
            ...$("iframe[data-src]").map((_, el) => $(el).attr("data-src")).get(),
            ...$("iframe[src]").map((_, el) => $(el).attr("src")).get(),
        ]
            .filter((u): u is string => typeof u === "string" && /^https?:\/\//i.test(u));

        const playerIframeUrls = await getPlayerIframeUrls(toonStreamIframeUrls);
        Cache.set(key, playerIframeUrls, EPISODE_IFRAMES_TTL, true);
