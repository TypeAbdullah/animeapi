import { movieScraper } from "./scrapers/movie.js";
import { seriesScraper } from "./scrapers/series.js";
...
export async function moviePlayer(slug: string) {
  ...
  const res = await movieScraper(slug)
...
export async function episodePlayer(slug: string) {
  ...
  const res = await seriesScraper(slug)
