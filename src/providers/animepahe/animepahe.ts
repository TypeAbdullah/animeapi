...
export class Animepahe {
  ...
  static async info(id: string): Promise<AnimeMeta | null> {
    try {
      ...
      let mal_id: number | undefined;
      let anilist_id: number | undefined;
      let anidb_id: number | undefined;
      ...
      $(".external-links a").each((_, el) => {
        ...
        try {
          ...
          } else if (url.includes("anilist.co/anime/")) {
            const m = url.match(/anime\/(\d+)/);
            if (m) anilist_id = parseInt(m[1]!, 10);
          } else if (url.includes("anidb.net/anime/")) {
            const m = url.match(/anime\/(\d+)/);
            if (m) anidb_id = parseInt(m[1]!, 10);
          }
        } catch { /* ignore */ }
      });
  ...
  private static async extractHls(
    ...
    const kwikHeadersObj: any = {};
    kwikHeadersRes.headers.forEach((v, k) => { kwikHeadersObj[k] = v; });
    const kwikLocation = getMapValue(
      JSON.stringify(kwikHeadersObj),
      "location",
    );
    ...
    while (statusCode !== 302 && attempts < 20) {
      const originalHeadersObj: any = {};
      originalRes.headers.forEach((v, k) => { originalHeadersObj[k] = v; });
      let cookie = originalHeadersObj["cookie"] ?? "";
      const kwikHeadersObj: any = {};
      kwikRes.headers.forEach((v, k) => { kwikHeadersObj[k] = v; });
      const setCookie = kwikHeadersObj["set-cookie"] ?? "";
      ...
      const postRes = await fetch(actionUrl, {
        ...
        headers: {
          Referer: kwikRes.url,
          Cookie: cookie,
          "User-Agent": originalHeadersObj["user-agent"] ?? USER_AGENT,
      ...
      attempts++;
      if (statusCode === 302) {
        const postHeadersObj: any = {};
        postRes.headers.forEach((v, k) => { postHeadersObj[k] = v; });
        location = getMapValue(
          JSON.stringify(postHeadersObj),
          "location",
        ) ?? "";
        break;
      }
